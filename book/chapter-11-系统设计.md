...
第 10 章定义了核心机制：玩家反复参与的规则化动作或决策单元——跳跃、射击、打出卡牌、安排生产。这些是**动词**或最小判断结构。但一个只有动词的游戏，除了按下按钮看角色蹦起来，什么都做不了。要让动词有意义，你需要**语法**——定义动词的时态（什么时候能跳）、宾语（跳过去干什么）、后果（跳错了会怎样）。这就是系统存在的必要性。
...
> **核心结论：Core Mechanic 定义玩家反复参与的动作/决策，System 定义这些参与如何产生状态、反馈和后续差异。** 没有足够系统语境的机制可能只是 Toy（玩具），第 5 章的 Toy First 原则是用最低成本验证适合单独验证的交互；当价值依赖目标、资源或长期状态时，原型应把最小必要系统一起纳入。Core Mechanic 是骨架层的参与单元，System 是骨架层更高维度的组织单元。**两者都由设计师设计，也都需要通过玩家测试验证。**
...
| 阶段 | 应该给 | 为什么 | 反面教材 |
|------|--------|--------|---------|
| **早期会话** | 先提供少量核心动作，并让玩家立即能用 | 确立"这个游戏是关于什么的" | 一开始同时暴露过多能力，玩家无法建立优先级 |
| **中期会话** | 按学习节奏给新能力或深能力（升级/变异/组合） | 在拓宽可能性和控制认知负荷之间平衡 | 长时间只有普攻，或新能力密度过高 |
| **后期会话** | 主要给深能力；必要时加入质变型新能力 | 维持新鲜感而不破坏已有理解 | 新能力让老内容重新产生策略价值，或使系统复杂度失控 |
...
| 步骤 | 做什么 | 独立游戏特别注意事项 |
|------|--------|---------------------|
| **1. 选择主货币** | 确定服务于核心决策的货币或资源 | 数量由玩法需要、信息负荷和内容规模决定；不要为了“像 F2P”或“像独立游戏”套用上限 |
...
| **4. 设计获取渠道** | 战斗掉落、任务奖励、探索发现、产出建筑 | 来源数量和可替代性服务于核心决策；稀缺资源是否需要多个来源，要看玩家路径和卡死风险 |
| **5. 设计消耗口** | 升级费、修理费、解锁费、合成费、购买 | 消耗应与来源、节奏和玩家目标匹配；来源和消耗数量不需要相等 |
| **6. 平衡供需** | 调掉落率、调价格，让资源流动服务于会话和长期目标 | 用电子表格模拟不同玩家路径、会话长度和情景，不要假设所有玩家每天玩同样时长 |
| **7. 周期性更新** | 新道具、新消耗口 | 独立游戏不需要 GaaS 频率。检查终局是否仍有有意义的资源用途；若没有，再考虑深度升级、收集目标或其他符合项目的消耗口。 |
...
| 检查项 | 通过标准 | 失败时的信号 |
|--------|---------|-------------|
| 来源多样性 | 核心货币的来源足以支持目标路径，并有单一来源失效时的风险预案 | 玩家抱怨“只能刷同一个副本”或被单一路径卡住 |
| 消耗口充足 | 消耗口、资源库存和玩家目标形成可解释关系 | 玩家到中后期“钱没地方花”或长期无法完成目标 |
| 净流入 | 来源与消耗在不同玩家路径和会话结构下符合成长目标 | 资源长期枯竭、闲置或通胀 |
| 边际效用递减 | 后续升级的成本和收益符合项目成长目标 | 玩家前期太简单、后期太难或升级失去意义 |
| 没有无限套利 | 所有交易路径中，A→B→C→A 不能产生净收益 | 玩家发现刷钱漏洞后经济崩溃 |
| 货币不闲置 | 资源库存和消耗窗口符合会话/长期目标 | 通关时货币大量闲置，或玩家长期无法使用资源 |
...
> **独立游戏的教训：** 经济系统不在于“有多少种货币”，而在于**每一种资源都有可理解的用途**和与核心决策相关的使用机会。少量相互竞争的消耗口可能比许多各管一摊的货币更易维护，但数量应由玩法需要决定。
...
> **实践经验：** 公式应根据数值范围、成长方向、可读性和玩法目标选择，并通过极端情景和玩家测试验证。混合公式可以避免某些减法公式的临界点，但不能保证在任何攻防比例下都有可玩性。
...
3. **Boss 的节奏编排 = 观察→防御→攻击循环的典型示范。** Boss 可以通过多种攻击模式、可读的间隔和明确的视觉/音效提示，逐步让玩家学习"看到这个信号 → 该这样反应 → 这里有攻击窗口"。具体模式数量和间隔应由 Boss 复杂度、目标难度和测试结果决定。
...
| 错误类型 | 表现 | 如何诊断 | 修复方法 |
|---------|------|---------|---------|
| **纯正反馈雪球** | 早期领先的玩家在项目预期的剩余会话中很难被追上 | 比较不同阶段的领先转化、策略空间和玩家感受 | 给落后方加速资源生成、降低领先方收入系数 |
| **纯负反馈兜底** | 玩家学会"先落后再反超"是最优策略 | 观察是否有玩家故意在早期输掉以获得更好的兜底 | 负反馈给予的是**翻盘机会**而非**翻盘保证**；兜底帮到持平为止，不要帮到反超 |
| **无反馈回路** | 比分随机波动，玩家感受不到自己的决策影响了结果 | 比较最终结果与中期行为、决策和玩家感受；不要用单一相关系数自动判定 | 增加可理解的积累、信息或机会，让更强的发挥在目标体验中得到回应 |
| **反直觉的负反馈** | 负反馈在玩家层面不可见 → 玩家感觉"系统在操控" | 玩家能说出"我觉得领先时好像更容易被打中"就是信号 | 让负反馈**可见且可理解**——Mario Kart 的道具是显式的，"你落后所以给你好道具"是玩家能接受的故事 |

> **核心原则：** *Rules of Play* 区分了放大差异和缩小差异的反馈回路。是否需要两者、各自强度以及终局方式，应由游戏的核心体验和会话结构决定；并非所有游戏都必须同时具备明显的正负反馈。
...
| **血肉（表现）** | 反馈是否让玩家**感知到**？落后时屏幕边缘变红？领先时音乐加速？正负反馈不应只存在于后台数值，也可以通过视听、动效、文本或空间变化表达。 |
...
**Vampire Survivors（强正反馈，弱负反馈）：** 这是正反馈主导设计的案例。击杀越多 → 经验越多 → 升级越快 → 武器越强 → 击杀更多。会话前段玩家承受压力，后段逐步获得压倒性力量；这个设计的爽感来自“被追着跑”到“我就是风暴”的成长弧。负反馈的缺失由该类型的会话终止和构筑结构共同承担，不应把固定时长照搬到其他项目。

> **不同强度的反馈回路都可能成立——前提是它们与目标体验、会话结构和终局方式一致。** 最危险的是**无意识地复刻别人的反馈系统**：把单人割草游戏的正反馈强度直接用于需要持续博弈的 PvP 格斗游戏，比赛节奏可能迅速失控。
...
- [ ] 选择核心成长曲线类型（线性/指数/对数/S曲线或自定义曲线），并按项目的早期、中期、后期会话画出能力值——各阶段是否服务于体验目标？
- [ ] 列出每种货币的来源渠道和消耗口，检查来源、消耗、库存和玩家路径是否匹配。
- [ ] 用电子表格跑不同会话长度、玩家路径和情景的经济模拟，检查资源是否支持核心决策。
- [ ] 战斗系统：明确列出攻击前摇、位移距离、收尾帧数等关键参数，并验证不同攻击在用途、风险和反馈上有差异。
- [ ] 确认战斗的防御机制数量和交互不会超过玩家可理解与团队可维护的范围；超出时先评估、合并或删减。
- [ ] 列出游戏的正反馈和负反馈各是什么，或说明为什么当前体验只需要其中一种。
...
| 不同会话阶段的成长变化低于玩家可感知且与体验目标匹配的程度 | 玩家感受不到成长，或成长节奏与类型不符 |
| 测试中玩家到中后期说"钱已经没用了" | 消耗口不够。你需要一个新的货币消耗方式 |
| 多名匹配目标的 playtest 玩家不知道某个攻击比另一个"好在哪" | 攻击差异化不够——不只是数值不同，手感/用途/场景必须不同 |
| 玩家能准确说出"我感觉领先时就容易输" | 你的负反馈在数值层面有，但表现层不可见，变成了"系统在耍我" |
| 早期领先在项目预期的剩余会话中几乎总能转化为胜利 | 正反馈可能过强，需要检查负反馈、策略空间和终局结构 |
...
| **骨架（机制与规则）** | 成长曲线数值是否经过电子表格验证？经济来源-消耗是否平衡？ | 不同会话长度、玩家路径和情景下，资源流动服务于核心决策 |
| **骨架（机制与规则）** | 伤害公式是否避免了"破防临界点"？反馈回路是否符合体验目标？ | 极端情况下反馈仍可解释，不会产生意外无敌/必败 |
