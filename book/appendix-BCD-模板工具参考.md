...
> **说明：** 以下所有模板均为**可填空结构**。复制到你的笔记本或文档工具中，按序填写。不要跳过任何空行——空行是你必须做出决策的位置。
...
| **Design Pillars（2-4条）** | 1. |
| | 2. |
| | 3. |
| | 4. |
| **Core Mechanics（核心机制，≤3个）** | 1. |
| | 2. |
| | 3. |
| **Genre（类型）** | |
...
> **用途：** 第2步（轻量设计概要）核心产出。限制在单页A4内。
> **原则：** 限制篇幅提升设计质量。能写进一页纸的才是你真的想清楚了的。
...
| 页 | 标题 | 填写内容 |
|----|------|---------|
| 1 | **游戏概述** | Elevator Pitch + Design Pillars（从一页纸升级）。最终版的一句话描述。 |
| 2 | **Core Loop 展开** | 完整核心循环箭头图，含所有资源流、转化节点、退出条件。附：单次循环预计时长。 |
| 3 | **机制清单** | 所有游戏机制的完整枚举。每个机制写：名称、触发条件、系统响应、与其他机制的交互、引入时机（第几关/第几分钟）。 |
| 4 | **内容类型清单** | 所有内容资产的分类枚举。如：敌人类（`Enemy_Melee`, `Enemy_Ranged`, `Boss`）、道具类、环境资产类、UI面板类。每类附数量估算。 |
| 5 | **关卡/进度结构** | Macro Chart：一张总表列出所有关卡/区域/章节的名称、引入机制、预计通关时长、难度曲线位置。附关卡解锁流程图。 |
...
| 10 | **Scope Budget** | 内容 × 数量 × 单位成本 = 总工时。时间预算总表（各阶段推荐时长和上限）。详细见 B.4 模板。 |
...
> **用途：** 第5步（垂直切片）的核心数据产出。用两个切片的真实耗时反推全量产能。
> **原则：** 第一切片测单件成本，第二切片测复用率。两数一除，真实产能就出来了。
...
| **切片2（第二个同类型内容）** | | |
| └ 切片2内容描述 | | 如"洞窟主题第一关，含2种新敌人 + 1个新 Boss + 2种新道具" |
| └ 切片2实际耗时（天） | ____ 天 | 复用管线后的完整时间 |
| └ 切片2涉及新内容类型数 | | |
| **产能分析** | | |
| └ 复用效率 | ____ % | 切片2耗时 / 切片1耗时 × 100%。<70% 说明管线良好，>85% 说明复用不足 |
| └ 单关平均耗时（天） | ____ 天 | 取切片2的单位成本 |
| └ 预计总关数 | | 从十页纸 Macro Chart 取 |
| └ 关卡生产总预估 | ____ 天 | 单关平均耗时 × 总关数 |
| └ 剩余内容生产预估 | ____ 天 | 非关卡内容（UI/叙事/营销）从 Scope Budget 取 |
| └ **全量生产总预估** | **____ 天** | |
| **判断** | | |
| └ 可用资源（人·天） | ____ 天 | 你的实际生产力上限 |
| └ 是否有缺口？ | 是 / 否 | 有缺口 → 砍内容或砍品质，现在决定，不等到生产一半 |
