...
> **这一节解决什么问题：** "怎么开始做一个关卡"是最令人瘫痪的问题。空白编辑器比空白画布更可怕——因为你不只要画，还要让空间"产生玩法"。本节提供从宏观世界地图到微观单关布局的四阶段流程，给你一个不可跳过的开工路径。
...
*Celeste* 证明了：**好的关卡设计不是空间大，是每个像素都有意图。**

### 关卡设计四阶段流程

*Practical Game Design* 第 9 章将关卡设计分解为五个阶段，本书将其凝练为四个不可跳过的步骤：

| 阶段 | 名称 | 产出 | 时间占比 | 核心动作 | 跳过此阶段的后果 |
|------|------|------|---------|---------|----------------|
| **1** | **概念（Concept）** | 一句话关卡目标 + 2-3 个核心体验关键词 | 10% | 明确这一关要玩家感受到什么、学会什么 | 关卡没有"灵魂"——只是素材的随机排列 |
| **2** | **草图（Sketch）** | 俯视平面图，标注玩法节拍、敌人位置、路径、关键物件 | 15% | 在纸上或绘图软件中画出完整的空间流程和玩法分布 | 灰盒阶段方向不明，反复推翻布局 |
| **3** | **灰盒（Graybox）** | 在引擎中用简单几何体搭建可玩版本，无美术资产 | 50% | 用方块和圆柱搭建完整空间，放入核心玩法和敌人，反复测试 | 美术返工——发现墙体位置不对时已经画好了贴图 |
| **4** | **迭代（Iteration）** | 替换美术、调数值、加反馈、修碰撞——直到达标 | 25% | 在可玩基础上抛光：光照、音效、VFX、难度微调 | 关卡"能玩"但"不好玩"——玩家说不出原因，就是不想继续 |

**灰盒为什么占了 50%？** 因为灰盒是"设计真正发生的阶段"。概念和草图是假设，灰盒是实验——只有在可玩版本中，你才能发现"这个走廊太长了""这个拐角的敌人玩家永远看不清""从 A 点到 B 点居然走了 40 秒但无事发生"。

**纪律：灰盒不通过，绝不开美术。** *Practical Game Design* 明确警告：在关卡布局验证之前就投入美术资产，是关卡设计最昂贵的错误之一。灰盒阶段应该允许你用 5 分钟移动一堵墙——但如果墙上已经画好了贴图，你会犹豫 5 小时。
...
| **呼吸点** | 各阶段之间 | 安全的过渡空间——无敌人、有资源、有景观。让玩家从上一个紧张段落中恢复 | 没有呼吸点——连续 15 分钟高强度后，玩家的注意力和操作精度都会崩溃 |
...
|---------|------|---------|---------|---------|
| **教学** | T | 1-2 | 30-90s | 安全环境、单一新机制、失败无惩罚 |
| **探索** | E | 1-3 | 60-180s | 多路径、隐藏物、景观展示 |
| **战斗** | C | 2-4 | 30-120s | 敌人配置、地形利用、资源消耗 |
| **解谜** | P | 2-4 | 60-300s | 环境交互、逻辑推理、无时间压力 |
| **平台跳跃** | PL | 2-5 | 15-60s | 精确操作、反复重试、短失败循环 |
| **叙事/对话** | N | 1 | 30-60s | 无操作或轻操作、情绪释放、信息传递 |
...
| **地标引导** | 远处的独特建筑/景观作为一个"北极星"——玩家始终知道自己在空间中的相对位置 | 在关卡入口处让地标可见；玩家移动时地标应保持在视野中；地标必须是该区域中独一无二的形状 | **Dark Souls** 的火祭场——从多个区域都能看到它，成为整个世界的空间锚点 |
...
| 法则 | 说明 | 自查问题 |
|------|------|---------|
| **多线索汇聚** | 不要只依赖一种引导——让光线 + 地标 + 敌人分布同时指向正确方向 | 如果我关掉 UI，玩家还能在 30 秒内找到路吗？ |
| **始终给玩家一个"远景目标"** | 玩家应该随时能看到一个"我要去那里"的方向 | 站在关卡的每个角落，能看到至少一个"下一步"的方向吗？ |
| **不要惩罚探索** | 走错路 = 发现一个隐藏物/一段景观/一个彩蛋，不是死胡同 + 3 个敌人 | 玩家走错方向时，给他的反馈是"哦，这也是个发现"还是"白走了，烦"？ |
| **分支路口的等级分明** | 三条路：主路最宽最亮（>），支路稍窄（可选奖励），回头路最不显眼 | 玩家在路口停留超过 3 秒还选不出方向吗？ |
...
> **这一节解决什么问题：** "想不出有趣关卡主题"是最常见的创意障碍。*通关！* 的作者 Scott Rogers 提供了一个直面问题的答案——你不是创造力枯竭，你是题材库不够大。本节列出的十大经典题材及其混搭方法，让你永远有起点可查。
...
- **景观和地标让你永远知道自己在哪**——灰姑娘城堡在乐园任何一个角落都能看到
...
- [ ] **检查节奏**——连续高强度不超过 3 个区块；Boss 前有预热；每 3-4 个区块有呼吸点
- [ ] **难度曲线用锯齿形**——引入新机制时降低已掌握机制的难度，给玩家"学习缓冲"
- [ ] **三种以上引导技术同时指向关键路径**——光线 + 地标 + 形状 = 三线汇聚
- [ ] **灰盒不通过，绝不开美术**——在方块阶段用至少 3 个外部测试者跑通关卡
- [ ] **每个分支路口有明确的主次区分**——主路最宽最亮，支路其次，死路最不显眼
- [ ] **远景目标始终可见**——站在关卡的每个角落，应该能看到至少一个方向是"我要去那里"
...
| 信号 | 诊断 | 修复方向 |
|------|------|---------|
| **测试者在同一位置停留超过 5 秒无效操作** | 引导失败——玩家不知道下一步做什么或往哪走 | 增加隐性引导（光线、形状、地标），或在该位置加一个微小的视觉线索（一抹暖色、一个亮着的灯） |
| **测试者连续死了 3 次以上在同一个位置，且未能调整策略** | 难度陡增——玩家没有足够的信息或时间形成新策略 | 在该位置之前增加一个"教学版"——同一机制的更简单版本，或增加失败后的提示（不是降低难度，而是降低信息差） |
...
| **灰盒测试通过率 > 90% 且测试时间 < 预期 50%** | 关卡太简单或太空洞——玩家飞过去了 | 增加支线内容（隐藏物、可选挑战）或提高核心路径的密度 |
| **玩家在路口左右看，然后随机选** | 引导的等级不明——所有路看起来一样 | 主路必须比支路宽至少 50%、亮至少 30% |
...
| **血肉（表现与动态）** | 空间引导（光线、色彩、地标、声音）是否让玩家无需思考就知道该往哪走？ | 测试者不会在一个区域停留超过 10 秒不知道去哪 |
...
| **灵魂（情感与意义）** | 通过关卡后，玩家感受到的是"终于结束了"还是"我还想再来一次"？ | 至少 50% 的测试者主动重玩或探索隐藏区域 |
