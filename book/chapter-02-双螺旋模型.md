...
Katie Salen 和 Eric Zimmerman 在 *Rules of Play* 中提出了一个根本性的原则：**你无法在运行游戏之前完全预测游玩体验。** 他们明确指出，游戏设计师不直接创造体验——你创造的是规则系统，体验是从规则与玩家交互中**涌现**出来的。这意味着无论你的设计文档写得多么详尽，仍需要用玩家与系统的交互检验关键假设。

这就是为什么 *Rules of Play* 强调**迭代设计（Iterative Design）**：Prototype → Playtest → Evaluate → Refine → Repeat。尽早出现可玩的验证形式是一种有用的项目纪律；“20% 规则”可以作为提醒，但应按项目规模、类型和最大不确定性调整，不能当作跨项目的硬门槛。
...
下面是一个示意场景：两个独立开发者面对同一个创意——"做一个关于穿越沙漠的生存游戏"。其中的数字和反馈只用于说明决策差异，不是排期或测试模板。
...
第 3-4 周: 数字原型。只有 3 种资源，灰盒地形，但核心循环完整
          找几位具有目标受众特征的玩家测试。反馈："很紧张，有点像赌博，好玩"
          也有人反馈："总觉得少点什么"
第 5 周: 回顾设计。发现"少点什么"是缺乏"意外事件"——不是更多资源类型
         加了一个"沙漠风暴随机事件"系统，回 Step 2 更新一页纸
```

开发者 B 的故事不是要求每个项目照搬的理想化排期，而是用来说明一种常见的探索方式。游戏的核心往往需要在设计与验证之间逐步澄清；设计文档是对当前假设和发现的记录，而不是对创造的永久规定。**你不会在挖到宝藏之前就写好宝藏的精确描述；你挖上一铲子，看看下面是什么，再决定下一铲子往哪儿挖。**
...
**类比：** 这就像编程中写实现代码。你从"最简单的能跑的版本"开始（最小可行代码），逐步加功能、加边界处理、加性能优化。但关键区别是：编程中的实现是朝着一个已知正确的接口努力，游戏原型实现的过程中**接口设计本身可能被证明是错的**——这就是为什么需要回流。

原型线的一个重要概念是 **Toy First（先做玩具）**，来自 Kyle Gabler 在 CMU Experimental Gameplay Project 中的核心经验：在加完整内容之前，先用低成本版本检查核心交互是否有吸引力。如果目标玩家在短时间内愿意主动重复、尝试变化或继续探索，这个“玩具”就有潜力成为好游戏的内核。无目标试玩是某些动作、物理和创造型玩法的有效实验，但对依赖情境、叙事、长期构筑或信息积累的游戏，应该连同最小必要情境一起测试。

原型线的另一个关键原则是 **Fake It（战略性造假）**：用最快的方法实现效果，不在非核心验证目标上浪费工程时间。光照用烘焙不用实时光追，AI 用假数据不用模式识别，UI 用占位符不用精心设计。**玩家看不到伟大工程——也不在乎。** 但注意："Fake"的是支撑系统（光照、AI、UI），不是你要验证的核心假设本身。如果你的核心是程序化生成关卡，那 关卡生成算法就是你正在验证的东西，不能用手工地图来"Fake"。
...
**原型 → 设计（反向修正）：** 原型线的测试结果回流到设计线，修正假设。如果目标玩家在多个测试条件下反复退缩而非进攻，那你的 Pillar“侵略性战斗”可能从根本上就错了——不一定只是数值问题，也可能是概念或反馈问题。这时应回到 Step 1 或 Step 2，重新定义并再次验证设计假设。
...
1. **步骤编号不在两条线之间一一对应。** 设计线的 Step 1 和 Step 2 之间没有独立的原型线步骤——原型线在 Step 1 可以是“关键体验片段的脑内推演”，在 Step 2 和 Step 3 之间是“纸上原型 → 数字原型”的连续流。双螺旋不是“设计一步、原型一步”的交替，而是两条线各自有自己节奏的并行推进。

2. **Step 3 是原型线的主场。** 这是原型线从纸上到数字、从 Toy 到 MVP 的集中爆发期。同时，Step 3 的测试结果也是设计线回流最剧烈的节点——原型数据可能推翻所有前面的假设。

3. **Step 4 是设计线的主场。** Core Loop 获得足够证据支持后，设计线需要从“方向探索”切换到“执行规划”。十页纸和 Scope Budget 是这个节点的核心产出。注意，在 Step 4 中，原型线也在工作——高风险新系统应先做与其复杂度相称的微原型验证。

4. **Step 5 是双线第一次"拧紧"的节点。** 垂切同时要求设计线给出可执行规格、原型线验证代表性风险。切片的测试结果（产能数据、管线可复用性、品质和系统整合）支持是否进入生产，但不能替代对所有内容类别的估算与复测。这是整个流程中风险最高的转折点之一。
...
- Step 7 时，设计可以是"处决回血从一个版本值调低"，原型则观察调节后玩家通关率、策略分布或主观体验如何变化。两条线几乎重叠；具体数值应以项目数据为准。

这就是**螺旋结构**：不是两条保持等距的平行线，而是一条从松散到紧密、但仍允许受控回流的收敛曲线。早期螺旋松——允许大范围探索和剧烈回流；晚期螺旋紧——变更成本更高，需要评估返工范围并重新验证，但如果核心证据被推翻，仍然可以回流。你可以把它想象成一根弹簧从顶部宽松的圈到底部紧密的圈。

**这个结构回答了一个关键问题：为什么早期更强调低成本实验，晚期更强调变更影响评估？** 因为早期停止或改方向的成本低，晚期变更的成本高；但成本差异不等于固定轮数或固定评分阈值。Kill Criteria 的作用是让证据、成本和替代方案透明，而不是替设计师自动按下停止键。
...
| ③ 原型验证 | 一页纸 GDD | 可玩原型（MVP） | 当前体验假设成立吗？ | 证据在目标玩家中持续否定，且低成本替代方案不足 → 回流或停止 |
| ④ 设计深化 | 验证后的体验结构 | 十页纸 + Scope Budget | 怎么做完？成本可控吗？ | 预算或依赖无法解释 → 砍内容、拆风险或回流 |
| ⑤ 垂直切片 | 十页纸 | 品质样本 + 产能数据 | 关键风险和内容产能可解释吗？ | 样本不具代表性、瓶颈不明或估算区间过宽 → 补样本、建管线或回流 |
| ⑥ 生产开发 | VS/HS 证据 + 十页纸 | 内容完成 + 生产模板 | 内容可复用？进度可预测？ | 范围漂移、速度偏差或新证据推翻设计 → 变更评估、重建或回流 |
| ⑦ 迭代打磨 | 完整游戏 | 可发布版本 | 体验和稳定性达到项目发布标准吗？ | 证据持续不改善或核心 Bug 不收敛 → 修复、裁减、回流或延期 |
...
| ⑥ 生产开发 | "能批量生产吗？如何控制变更？" | 冻结范围、建模板，持续产出并记录变更 |
...
flowchart TD
    ...
    Q3 -->|表达/实现问题| FIX2["修改表达或机制<br/>重复低成本实验"]
    Q3 -->|价值主张仍有潜力| FIX3["更换实现方式<br/>记录假设后复测"]
    Q3 -->|多轮证据持续否定| STOP2["🛑 回流概念或停止"]
    Q3 -->|证据支持继续| S4["④ 十页纸深化"]
    FIX2 --> S2
    FIX3 --> S3
    S4 --> Q4{"Scope Budget 可控？<br/>系统可收敛？"}
    Q4 -->|估算或依赖不可解释| FIX4["补样本、拆风险<br/>或砍内容"]
    FIX4 --> S4
    Q4 -->|是| S5["⑤ 垂直切片（VS）"]
    S5 --> Q5{"管线可行？<br/>代表性样本的产能与风险可解释？"}
    Q5 -->|否| FIX5["补代表性样本<br/>重建管线或回流"]
    Q5 -->|是| S6["⑥ 生产开发"]
    S6 --> Q6{"内容可复用？<br/>进度可预测？"}
    Q6 -->|否| FIX6["重建模板、收缩范围<br/>或做变更评估"]
    FIX6 --> S6
    Q6 -->|是| S7["⑦ 迭代打磨"]
    S7 --> Q7{"体验一致？<br/>Bug 收敛？"}
    Q7 -->|证据不足或体验未达标| FIX7["继续定向测试<br/>或受控回流"]
    Q7 -->|核心 Bug 不收敛| FIX8["裁减高风险系统<br/>或延期"]
    Q7 -->|达到项目发布标准| PUBLISH["✅ 发布"]
...
**1. 前进（Advance）：证据足以支撑下一项投资 → 进入下一步**

每一步都有明确的"门控标准"（Gate）。你不因为是"时间到了"而前进，而是因为"回答了这个步骤的核心问题"：

- Step 1 → 2：你找到了明确卖点和目标受众
- Step 2 → 3：一页纸的可读性测试通过，Scope 可控
- Step 3 → 4：与项目体验承诺相关的循环在目标玩家测试中得到支持，主要未知数已列出
- Step 4 → 5：范围、依赖和高风险系统有可解释的估算与验证计划
- Step 5 → 6：代表性内容的品质、管线和产能区间足以支持生产决策
- Step 6 → 7：范围受控、内容可复用，剩余变更有记录和批准流程
- Step 7 → 发布：体验、稳定性、兼容性和商业发布标准达到项目自定义门槛
...
- **Step 3 → Step 3：** 当前实现或表达失败，但价值主张仍有潜力，换一个低成本实现方式并重新验证。例如玩家喜欢近身风险感，但当前的回血机制让他们只会保守躲避。
- **Step 3 → Step 2：** 玩法有潜力但 Pillars、目标或范围表达不清，需要修改设计概要后再做原型。
- **Step 3 → Step 1：** 多种低成本实现都无法支持核心价值主张，或目标玩家持续表现出无兴趣，才回到概念层或停止。
- **Step 5 → Step 3：** 垂切漂亮但不好玩，说明视觉掩盖了玩法问题。需要回去重做原型。
- **Step 6/7 → 前序步骤：** 新证据推翻核心假设时，先评估受影响的内容和成本，再决定局部回流、裁减范围或延期。
...
Kill Criteria 用预先写好的证据条件和停止/转向规则，帮助你在情绪卷入之前做决定。它的价值不是把体验变成自动化测试，而是让假设、证据、样本局限、成本和替代方案透明，避免把偶然数据或主观感觉伪装成确定结论。
...
- Step 1-3 的判断重点是用低成本实验减少概念和体验不确定性；证据持续否定价值主张时，才考虑停止或转向。
- Step 4-5 的判断重点是范围、依赖、品质和管线的可解释性；可以砍内容、补样本、重建管线或回流。
- Step 6-7 的判断重点是变更成本和发布标准；应冻结无依据的范围扩张，但保留基于证据的受控设计变更。

这个渐进松弛的 Kill Criteria 体系，就是螺旋收敛结构在决策层面的映射。

**时间与资源是风险信号，不是统一上限：**

Kill Criteria 不仅包含“质量条件”（玩家是否觉得好玩），也包含时间、预算、依赖和机会成本。每个项目应为阶段设定自己的检查点和资源边界；触碰检查点时先诊断实验设计、实现方式、玩家分群、范围和价值主张，再决定继续、换方案、回流、收缩或停止。原型是获取信息的工具，不是推迟决策的借口，但复杂项目也不应因为超过某个统一天数就自动判死。
...
双螺旋模型的基本立场是：**市场校验不是开发之后才开始的推广活动，而是与设计、原型平行的证据线，从 Step 1 就开始运行；但不同阶段的市场信号强度和含义不同，不能把传播数据直接等同于玩法或需求结论。**
...
**Step 3 的社交媒体测试是传播信号，不是玩法验证。** 在原型阶段，可以在合适的平台发布短 GIF 或视频，观察素材是否触达目标受众。零互动可能来自曝光不足、受众不匹配、素材表达或平台分发问题，也可能提示卖点不清；它不能单独证明没人需要这个游戏。应把曝光、观看、点击、评论和目标玩家试玩/访谈放在同一条证据链中。

**Step 5 的商店页面是市场线的关键转折点。** 垂直切片或其他代表性样本产出可用素材后，可以创建页面并开始收集愿望单。愿望单是传播和购买意向的中间信号，必须结合页面曝光、点击、流量来源、开发阶段、品类和价格解释；它不能单独证明玩法或需求成立。

**Step 6 的 Demo 和展会是市场线的集中验证阶段。** Steam Next Fest（新品节）等活动可能带来曝光、试玩和反馈，但效果受平台规则、时间、品类、素材、受众和支持能力影响，不是所有项目的“唯一窗口”。参与前先定义要验证的假设、Demo 的最小内容、维护成本和后续行动；Demo 时长按游戏类型和验证目标决定。
...
交互结果：
  - 如果在足够曝光且目标受众匹配的情况下互动、试玩和复述都积极 → 市场假设得到初步支持 → 继续深化
  - 如果曝光不足或受众不匹配 → 先修正渠道和素材，再观察市场假设
  - 如果多版素材在可比条件下都缺少有效反应，且试玩/访谈也不支持购买理由 → 重新评估卖点或定位
...
- Step 3：社交媒体互动是传播信号——它能帮助测试素材表达和触达，但"点赞"与"购买"之间仍有巨大鸿沟。
- Step 5：愿望单数据是转化漏斗中的中间信号——只有在曝光、阶段、品类、价格和流量来源相近时才适合比较。
- Step 6-7：Demo 完成、试玩后的购买意向、主播覆盖和愿望单变化提供更接近购买的信号，但仍受渠道、价格、时机和发布质量影响，不能直接预测销量。

这个信号递增的逻辑，与设计线从"宽松假设"到"精确参数"的螺旋收敛是一致的。市场线和设计线共享同一套收敛逻辑——这就是为什么它们是同一根螺旋的不同股线，而不是三条独立流程。

### 市场 Kill Criteria

与设计和原型一样，市场线也有自己的退出条件。这些条件与“游戏好不好玩”相关但不等同——一个好玩的原型可能尚未找到受众或渠道，也可能需要重新定位；市场信号应帮助你调整素材、定位、价格或范围，而不能仅凭单一指标替玩法下结论。

| 触发步骤 | 市场 Kill Criteria | 行动 |
|---------|-------------------|------|
| Step 1 | 经过检索、访谈和定位实验仍找不到可触达且有相关需求的目标受众 | 回到受众、价值主张或渠道假设；若替代假设也缺乏支持，再考虑停止 |
| Step 1 | 所有竞品在定位/品质/价格上明显优于你 | 重新思考差异化 |
| Step 3 | 在已知曝光量下，目标受众对多版素材都没有有效反应 | 检查渠道、素材、受众和卖点；与试玩和访谈交叉验证 |
| Step 5 | 在可比曝光、阶段、品类和流量来源下，页面转化持续偏低 | 重新审视页面、定位、价格和市场假设；不单独 Kill 玩法 |
| Step 6 | Demo 玩家反馈"不值得买"的比例显著高 | 回到 Step 5 或重新评估可行性 |
| Step 7 | 在已记录曝光、流量来源、价格和品类条件后，预发布转化信号仍不足以支持商业目标 | 结合试玩、购买意向、页面漏斗和资源情景，考虑延期、重新定位、缩小范围或停止 |
...
- [ ] **写一句话的市场假设。** 不管你现在在哪个阶段，填空："我相信______会因为______而关注、尝试或购买这款游戏。" 如果填不出来，先补齐假设、受众和证据来源。
- [ ] **确认你理解了 Kill Criteria 不等于"鼓励放弃"。** Kill Criteria 帮助你在证据、成本和替代方案透明的情况下决定继续、回流、收缩或停止，而不是预先鼓励某一种结果。
...
3. **螺旋不是平行线，而是收敛曲线。** 从 Step 1 到 Step 7，设计和原型之间的"距离"不断缩小。早期螺旋松——允许大范围探索和剧烈回流。晚期螺旋紧——变更需要评估返工范围、依赖和重新验证成本，但核心证据被推翻时仍可受控回流。Kill Criteria 的判断也应随证据和成本变化，而不是机械套用固定门槛。

4. **七步流程是一台"带风险决策的状态机"。** 每一步有明确的输入、输出和核心问题。步骤之间通过 Gate 标准前进，通过 Kill Criteria 回流、缩减、暂停或停止。Kill Criteria 的价值是帮助你在什么证据条件下重新评估，而不是把任意数字变成必须执行的命令。
...
| ⑥ 生产开发 | 稳定基线、控制变更，建模板并批量生产 | 持续产出内容资产，保持反馈质量 | 确保内容一致性不稀释情感体验 |
